...
local Revenue = TSM.Viewer:NewPackage("Revenue")
...
function Revenue:Draw(container)
	...
	tabGroup:SetCallback("OnGroupSelected", function(self, _, value)
		...
		elseif value == 3 then
			local stCols = TSMCore.db.global.accountingOptions.priceFormat == "avg" and ITEM_RESALE_ST_COLS_AVG or ITEM_RESALE_ST_COLS_TOTAL
	...
	for itemString, data in pairs(TSM.items) do
		if not TSM.ViewerUtil:IsItemFiltered(itemString, filters) then
			...
			if buyNum > 0 and sellNum > 0 then
				...
				local profitPercent = TSMAPI_FOUR.Util.Round(100 * profit / avgBuy)
				local color = profit > 0 and "|cff00ff00" or "|cffff0000"
				profitText = TSMAPI_FOUR.Money.ToString(profit, color) .. " (" .. color .. profitPercent .. "%|r)"
				isValidItem = true
			end
			...
			if isValidItem then
				...
				local row = {
					cols = {
						...
						{
							value = TSMAPI_FOUR.Money.ToString(avgSell),
							sortArg = avgSell,
						},
						{
							value = buyNum,
						...
						{
							value = TSMAPI_FOUR.Money.ToString(avgBuy),
							sortArg = avgBuy,
						},
						{
							value = profitText,
