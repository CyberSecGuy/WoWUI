...
local Inbox = TSM:NewPackage("Inbox")
...
function Inbox:OnEnable()
	TSMAPI_FOUR.Event.Register("MAIL_SHOW", private.MAIL_SHOW)
	TSMAPI_FOUR.Event.Register("MAIL_INBOX_UPDATE", function() TSMAPI.Delay:AfterTime("mailingInboxUpdate", 0.2, private.InboxUpdate) end)
	TSMAPI_FOUR.Event.Register("MAIL_CLOSED", private.MAIL_CLOSED)
end
...
local function CacheFrameOnUpdate(self, elapsed)
	if not private.waitingForData then
		...
		if seconds <= 0 then
			...
			if TSMCore.db.global.mailingOptions.autoCheck then
...
function private:UpdateTopLabel()
	...
	if nextRefresh then
		if numMail == 0 and TSMCore.db.global.mailingOptions.showReloadBtn then
...
function private:CanLootMailIndex(index, force)
	...
	if force or not TSMCore.db.global.mailingOptions.keepMailSpace or TSMCore.db.global.mailingOptions.keepMailSpace == 0 then
		...
		if remainingSpace >= TSMCore.db.global.mailingOptions.keepMailSpace then
...
function private:ShouldDeleteMail(index)
	...
	if not TSMCore.db.global.mailingOptions.deleteEmptyNPCMail then return false end
...
function private:PrintOpenMailMessage(index)
	if not TSMCore.db.global.mailingOptions.inboxMessages then return end
...
function private.OpenMailThread(self)
	...
	if private.keepFreeSlots then
		TSM:Printf(L["Stopped opening mail to keep %d slots free."], TSMCore.db.global.mailingOptions.keepMailSpace)
	...
	-- play sound
	TSMAPI:DoPlaySound(TSMCore.db.global.mailingOptions.openMailSound)
end
...
function private:MailThreadDone()
	...
	if private.moneyCollected > 0 and TSMCore.db.global.mailingOptions.displayMoneyCollected then
