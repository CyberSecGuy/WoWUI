...
function private.SellThread(self)
	...
	for _, bag, slot, itemString, quantity in TSMAPI.Inventory:BagIterator(true,true) do
...
function private:DoneSelling()
	...
	if TSMCore.db.global.vendoringOptions.displayMoneyCollected then
...
function private.BuyThread(self)
	...
	-- Determine quantity of items in inventory.
	for _, _, _, itemString, quantity in TSMAPI.Inventory:BagIterator() do
		inventoryItems[itemString] = (inventoryItems[itemString] or 0) + quantity
	end
