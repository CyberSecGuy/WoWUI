...
function Tooltips.Register(module, defaults, callbackLoad, callbackOptions)
...
function Tooltips:RegisterInfo(module, info)
	Tooltips.Register(module, info.defaults, info.callbackLoad, info.callbackOptions)
end
	...
	if private.tooltipLines.itemString ~= itemString or private.tooltipLines.quantity ~= quantity or (private.tooltipLines.lastUpdate + 5) < GetTime() then
		...
		if InCombatLockdown() then
			...
			for _, info in ipairs(private.tooltipInfo) do
				...
				if TSM.db.global.tooltipOptions.moduleTooltips[info.module] then
...
function Tooltips:LoadOptions(parent, moduleIndex)
	...
	if not moduleIndex then
		...
		if not TSM.db.global.tooltipOptions.moduleTooltips[info.module] then
			local keys = {}
			for key in pairs(TSM.db.global.tooltipOptions.moduleTooltips) do
				tinsert(keys, key)
			end
			TSMAPI:Assert(false, format("No tooltip info for %s (%s)", tostring(info.module), table.concat(keys, ",")))
		end
		info.callbackOptions(parent, TSM.db.global.tooltipOptions.moduleTooltips[info.module])
	end
end
...
function private:DrawTooltipGeneral(container)
					...
					children = {
						{
							...
							settingInfo = {TSM.db.global.tooltipOptions, "tooltipPriceFormat"},
							relativeWidth = 0.35,
						...
						},
						{
							...
							settingInfo = {TSM.db.global.tooltipOptions, "embeddedTooltip"},
							relativeWidth = 0.29,
						...
						},
						{
							...
							settingInfo = {TSM.db.global.tooltipOptions, "tooltipShowModifier"},
							relativeWidth = 0.35,
						...
						},
						{
							...
							settingInfo = {TSM.db.global.tooltipOptions, "inventoryTooltipFormat"},
							relativeWidth = 0.5,
						...
						},
						{
							...
							relativeWidth = 0.49,
							settingInfo = {TSM.db.global.tooltipOptions, "groupNameTooltip"},
						},
						{
							type = "Dropdown",
							...
							multiselect = true,
							settingInfo = {TSM.db.global.tooltipOptions, "operationTooltips"},
							relativeWidth = 0.5,
						...
						},
						{
							...
							settingInfo = { TSM.db.global.tooltipOptions, "vendorBuyTooltip" },
							relativeWidth = 0.5,
						...
						},
						{
							...
							settingInfo = { TSM.db.global.tooltipOptions, "vendorSellTooltip" },
							relativeWidth = 0.49,
					...
					children = {
						{
							...
							settingInfo = {TSM.db.global.coreOptions, "destroyValueSource"},
							list = priceSources,
							relativeWidth = 0.5,
						...
						},
						{
							...
							settingInfo = { TSM.db.global.tooltipOptions, "detailedDestroyTooltip" },
							relativeWidth = 0.49,
						...
						},
						{
							...
							settingInfo = { TSM.db.global.tooltipOptions, "millTooltip" },
							relativeWidth = 0.5,
						...
						},
						{
							...
							settingInfo = { TSM.db.global.tooltipOptions, "prospectTooltip" },
							relativeWidth = 0.5,
						...
						},
						{
							...
							settingInfo = { TSM.db.global.tooltipOptions, "deTooltip" },
							relativeWidth = 0.5,
						...
						},
						{
							...
							settingInfo = { TSM.db.global.tooltipOptions, "transformTooltip" },
							relativeWidth = 0.5,
	...
	if next(TSM.db.global.userData.customPriceSources) then
		local inlineGroup = {
			type = "InlineGroup",
			layout = "flow",
		...
		for name in pairs(TSM.db.global.userData.customPriceSources) do
			local checkbox = {
				...
				label = name,
				relativeWidth = 0.5,
				settingInfo = { TSM.db.global.tooltipOptions.customPriceTooltips, name },
